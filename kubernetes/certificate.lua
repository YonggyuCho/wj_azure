...
function _M.call()
  local cert_path = "/mnt/secrets-store/yg-cert.crt"
  local key_path = "/mnt/secrets-store/yg-cert.key"

  -- 인증서 파일 로드
  local cert_file = io.open(cert_path, "r")
  if not cert_file then
    ngx.log(ngx.ERR, "Failed to open certificate file: " .. cert_path)
    return ngx.exit(ngx.ERROR)
  end
  local pem_cert = cert_file:read("*all")
  cert_file:close()
  ...
  local pem_key = key_file:read("*all")
  key_file:close()

  -- 기존 인증서 제거
  local clear_ok, clear_err = ssl.clear_certs()
  if not clear_ok then
    ngx.log(ngx.ERR, "Failed to clear existing certificates: " .. clear_err)
    return ngx.exit(ngx.ERROR)
  end

  -- 인증서 및 키를 DER 형식으로 변환
  local der_cert, der_priv_key, der_err = get_der_cert_and_priv_key(pem_cert .. pem_key)
  if der_err then
  ...
  ngx.log(ngx.INFO, "Custom certificate loaded successfully!")
end
